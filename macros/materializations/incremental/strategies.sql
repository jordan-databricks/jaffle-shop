...
{% macro create_table_with_identity(source_relation, identity_start=1) %}
    {%- set identifier_with_identity = source_relation.identifier ~ '_with_identity' -%}
    {%- set relation_with_identity = api.Relation.create(
    ...
    {%- set columns = adapter.get_columns_in_relation(source_relation) | rejectattr("name", "equalto", "db_id") -%}
    {%- set cols_csv = columns | map(attribute='quoted') | join(', ') -%}
   
    {{ log("Creating identity table " ~ identifier_with_identity ~ " in schema " ~ source_relation.schema ~ " in database " ~ source_relation.database, info=True) }}
    {%- call statement('create identity table') -%}
        create table {{relation_with_identity}} (
